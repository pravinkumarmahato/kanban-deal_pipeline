...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Assuming response has access_token or token
      await login(data.access_token || data.token);
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-xl shadow-lg border border-slate-100">
        <div className="text-center">
          <h2 className="mt-6 text-3xl font-extrabold text-slate-900">Kanban Board</h2>
          <p className="mt-2 text-sm text-slate-600">Investment Pipeline Management</p>
        </div>
